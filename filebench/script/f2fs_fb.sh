...
mount_path=/f2fs_mount_point

meta_bench_path=/home/test/f2fsj/filebench/meta_data_only/f2fs_fb
meta_data_bench_path=/home/test/f2fsj/filebench/data_and_meta_data/f2fs_fb
realwork_bench_path=/home/test/f2fsj/filebench/real_workloads/f2fs_fb
...
function format_fs(){
    ...
    if [ $? -eq 0 ]; then
        echo -e "\e[32m format device with f2fs success\e[0m"
        return 0
    else
        echo "format device with f2fs fail"
        return 1
    fi
}
...
function mount_fs(){
    sudo modprobe f2fs
    if [ $? -eq 0 ]; then
        echo -e "\e[32m insmod f2fs.ko success\e[0m"
    else
        echo "insmod f2fs.ko fail"
        return 1
    fi

    sudo mount -t f2fs -o loop $dev_path $mount_path
    if [ $? -eq 0 ]; then
...
function reset(){
    ...
    if [ $? -eq 0 ]; then
        echo -e "\e[32m umount f2fs in $mount_path\e[0m"
        #return 0
    else
        echo "umount f2fs in $mount_path fail"
        return 1
    fi
...
fs_test
echo -e "\e[32m Begin to umount, $(date) \e[0m"
reset
