...
mount_path=/j_f2fs_mount_point

meta_bench_path=/home/test/f2fsj/filebench/meta_data_only/j_f2fs_fb
meta_data_bench_path=/home/test/f2fsj/filebench/data_and_meta_data/j_f2fs_fb
realwork_bench_path=/home/test/f2fsj/filebench/real_workloads/j_f2fs_fb

test_case=$1

function format_fs(){
    sudo mkfs.f2fs -f $dev_path
    if [ $? -eq 0 ]; then
        echo -e "\e[32m format device with f2fsj success\e[0m"
        return 0
    else
        echo "format device with f2fsj fail"
        return 1
    fi
}
...
function mount_fs(){
    sudo modprobe f2fsj
    if [ $? -eq 0 ]; then
        echo -e "\e[32m insmod f2fsj.ko success\e[0m"
        sudo sh -c 'echo 3 > /proc/sys/vm/drop_caches'
    else
        echo "insmod f2fsj.ko fail"
        return 1
    fi

    sudo mount -t f2fsj -o loop $dev_path $mount_path
    if [ $? -eq 0 ]; then
        echo -e "\e[32m mount f2fsj in $mount_path\e[0m"
        mount | grep $dev_path
        return 0
    else
        echo "mount f2fsj in $mount_path fail"
        return 1
    fi
}
...
function reset(){
    ...
    if [ $? -eq 0 ]; then
        echo -e "\e[32m umount f2fsj in $mount_path\e[0m"
        #return 0
    else
        echo "umount f2fsj in $mount_path fail"
        return 1
    fi
    sudo rmmod f2fsj
    if [ $? -eq 0 ]; then
        echo -e "\e[32m rmmod f2fsj in $mount_path\e[0m"
        return 0
    else
        echo "rmmod f2fsj in $mount_path fail"
        return 1
    fi
}
