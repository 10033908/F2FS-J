...
retry:
	...
	while ((nr_pages = pagevec_lookup_tag(&pvec, NODE_MAPPING(sbi), &index,
		...
		for (i = 0; i < nr_pages; i++) {
            ...
            // just for roll forward recovery test - case5
#if 0
            if (is_flush_for_rollforward(page, 4))
            {
                INFO_REPORT("Already flush dirty inode page ino-[%d] by WB, this moment is_dirty %d\n", 
                            ino_of_node(page), PageDirty(page));
            }
#endif
...
continue_unlock:
			...
			if (!PageDirty(page) && page != last_page) {
				/* someone wrote it for us */
                INFO_REPORT("Page (ino %d) is not dirty %d, last_page %p, skip tagging\n", ino_of_node(page), PageDirty(page), last_page);
				goto continue_unlock;
			}

			f2fs_wait_on_page_writeback(page, NODE, true, true);

            //printk("fsync write dirty page ino [%d]\n", ino_of_node(page));
			...
			if (!atomic || page == last_page) {
                ...
                INFO_REPORT("fsync is tagging, atomic %d, is_last_page %d, inode %d, nid %d\n",
                atomic, page == last_page, ino, nid_of_node(page));
    ...
    INFO_REPORT("Fsync node pages end\n", nr_pages);
...
continue_unlock:
			...
			if (ino_of_node(page) != nid_of_node(page))
            {
				goto continue_unlock;
            }

			if (!PageDirty(page) && page != NULL) {
				/* someone wrote it for us */
                INFO_REPORT("Somwone wrote inode page when kernel WB\n");
				goto continue_unlock;
			}

            INFO_REPORT("flush dirty inode page, ino is %d, nid is %d\n", ino_of_node(page), nid_of_node(page));

			f2fs_wait_on_page_writeback(page, NODE, true, true);

            set_already_flush_mark(page, 1);

			if (!clear_page_dirty_for_io(page))
            {
                INFO_REPORT("clear dirty flag for inode page [%d] failed, is_dirty %d\n", ino_of_node(page), PageDirty(page));
		    	goto continue_unlock;
            }
            else
            {
                INFO_REPORT("clear dirty flag for inode page [%d] success, is_dirty %d\n", ino_of_node(page), PageDirty(page));
            }

			ret = __write_node_page(page,
						false,
						&submitted, wbc, true,
						APP_WRITE_IO, seq_id);
			if (ret) {
...
next_step:
    ...
    //INFO_REPORT("WriteBack node pages\n");
